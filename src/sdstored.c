...
void updateTarefa(int sigin){
    if(DEBUG) write(1, "Received update\n", 17);
    int pipeClose = open("tmp/fecharTarefa", O_RDONLY), res, num;

    if(pipeClose > 0){
        char num_string[MAX_BUFFER];
        res = 0;
        while (read(pipeClose,num_string+res,1) > 0) {res++;}

        if(DEBUG) {
            writeTerminal("Closing");
...
int main(int argc, char **argv) {
    ...
    while(1) {
        int serverPipe = open("tmp/servidor", O_RDONLY) , res = 0;
        char clientPID[MAX_BUFFER];
        char buff_cliente[MAX_BUFFER];

      //  int res = 0;
        while (read(serverPipe, clientPID+res,1) > 0)  
            res++;
        ...
        if(validOption == 0){
            writeToClient(fPipeClient, "No valid option\n");
